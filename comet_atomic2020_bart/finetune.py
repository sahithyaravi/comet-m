...
def init_attribute_embeddings(model, tokenizer, special_tokens):
    ...
    https://github.com/mbforbes/social-chemistry-101/blob/bf47678b355e8e103d37fc38e1fd7e9cb2460025/sc/model/generative.py#L434
    """
    combine_map = { 
        "xReason":["xReason", "xWant"],
        "Causes":["Causes", "xEffect"],
        "HasPrerequisite": ["HasPrerequisite", "xNeed"],
        "isBefore": ["isBefore" ,"HasFirstSubevent"],
        "isAfter": ["isAfter", "xReact"]
        }
    ...
    for word in special_tokens:
        ...
        if word in combine_map:
            other_words = combine_map[word]
            ...
            print(
                f"\n Setting w[{index}] = {word} to the average of {other_words} ({other_indices})"
            )
            embeddings.weight.data[index, :] = vec.cuda()
...
class SummarizationModule(BaseTransformer):
    ...
    def train_dataloader(self) -> DataLoader:
        print(self.hparams.warmup_steps)
    ...
    def add_model_specific_args(parser, root_dir):
        ...
        parser.add_argument("--sortish_sampler", action="store_true", default=False) # we set this to true, sorts by length
...
def main(args, model=None) -> SummarizationModule:
    ...
    if args.atomic:
        ...
        # init_attribute_embeddings(model.model, model.tokenizer, additional_tokens_list) => resulted in worse performance
